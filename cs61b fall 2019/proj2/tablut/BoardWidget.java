...
class BoardWidget extends Pad {
    ...
    private void drawGrid(Graphics2D g) {
        ...
        for (Square t:ADJACENT_THRONE) {
            if (t != null) {
                g.fillRect(cx(t), cy(t), SQUARE_SIDE, SQUARE_SIDE);
            }
        }
    }
    ...
    private void drawPiece(Graphics2D g, Square s) {
        Piece temp = _board.get(s);
        if (temp == WHITE) {
            g.setColor(Color.WHITE);
            g.fillArc(cx(s), cy(s), SQUARE_SIDE - 5, SQUARE_SIDE - 5, 0, ANGLE);
        }
        if (temp == BLACK) {
