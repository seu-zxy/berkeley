...
public class MatrixUtils {
     ...
     *
     */
    public static double findmin(double[][] m,int h,int k){
        int line=m.length;
        int row=m[0].length;
        if(k==0)
        {
            if(m[h-1][k]<m[h-1][k+1])
                return m[h-1][k];
            else
                return m[h-1][k+1];
        }
        if(k==row-1){
            if(m[h-1][k]<m[h-1][k-1])
                return m[h-1][k];
            else return m[h-1][k-1];
        }
        if(k>0&&k<row-1){
            double min=m[h-1][k-1];
            if(min>m[h-1][k])
                min=m[h-1][k];
            if(min>m[h-1][k+1])
                min=m[h-1][k+1];
        ...
        for(l=0;l<line;l++){
            for(r=0;r<row;r++){
                if(l==0)
                    ...
                    temp[l][r]=findmin(temp,l,r)+m[l][r];
                }
            }
        }
    ...
    public static double[][]  transverse(double[][] m){
        int line=m.length;
        int row=m[0].length;
        double[][] temp=new double[row][line];
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        ...
        return accumulateVertical(m); //your code here
        else {
            double[][] temp=transverse(m);
            temp=accumulateVertical(temp);
            temp=transverse(temp);
            return temp;
        }
    }
