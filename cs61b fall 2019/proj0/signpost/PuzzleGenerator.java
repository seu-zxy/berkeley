        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                if (sq.successors() != null) {
                    int hh = sq.successors().size();
                    for (nextnum = 0; nextnum < hh; nextnum++) {
                        Place temp = sq.successors().get(nextnum);
                        Sq temps = model.get(temp);
                        if (sq.connectable(temps) && temps.sequenceNum() != 1) {
                            nFound++;
                            found = temps;
                            int t1 = sq.sequenceNum();
                            if (t1 != 0 && temps.sequenceNum() != 0) {
                                nFound = 1;
                                break;
                            }
                        }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (prenum = 0; prenum < sq.predecessors().size(); prenum++) {
                    Place temp = sq.predecessors().get(prenum);
                    Sq temps = model.get(temp);
                    if (sq.connectable(temps) && temps.sequenceNum() != w * h) {
                        nFound++;
                        found = temps;
                        if (sq.sequenceNum() != 0 && temps.sequenceNum() != 0) {
                            nFound = 1;
                            break;
                        }
                    }
