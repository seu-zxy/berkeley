...
public class ArrayHeap<T> {
    ...
    private int min(int index1, int index2) {
        double temp1 = getNode(index1).priority();
        double temp2 = getNode(index2).priority();
        if (temp1 < temp2) {
            return index1;
        }
        return index2;
    ...
    public Node peek() {
        ...
        int t1 = 1;
        double min = contents.get(1).priority();
        for (int i = 2; i < contents.size(); i++) {
            t1 = min(t1,i);
        }
        return contents.get(t1);
    }
    ...
    private void bubbleDown(int index) {
        int t1 = getLeftOf(index);
        int t2 = getRightOf(index);
        while ((getNode(t1) != null && min(t1,index) == t1) || (getNode(t2) != null && min(t2,index) == t2)) {
            if (getNode(t1) != null && getNode(t2) != null && min(t1,index) == t1 && min(t2,index) == t2){
                int t3 = min(t1,t2);
                swap(index,t3);
                index = t3;
                t1 = getLeftOf(index);
                t2 = getRightOf(index);
            } else if (getNode(t1) != null && min(t1,index) == t1) {
                swap(t1, index);
                index = t1;
                t1 = getLeftOf(index);
                t2 = getRightOf(index);
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i < contents.size(); i++) {
            if (contents.get(i).item().equals(item)) {
                Node val = new Node(item,priority);
                setNode(i,val);
            }
        }
    }
