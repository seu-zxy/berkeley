...
public class CompoundInterest {
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        double as=presentValue;
        for(int i=0;i<targetYear-THIS_YEAR;i++)
            as=as*(1+rate/100)*(1-inflationRate/100);
        return as;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
         ...
         double k=perYear;
         for(int i=0;i<targetYear-THIS_YEAR;i++)
            k=k*(1+rate/100)+perYear;
            k=Math.pow((1-inflationRate/100),targetYear-THIS_YEAR)*k;
        return k;
    }
    ...
    static void printDollarFV(int targetYear, double returnRate,
                              double inflationRate) {
        double nominalDollarValue =Math.pow((1+returnRate/100),targetYear-THIS_YEAR) ; // replace 0 with your code
        double realDollarValue = Math.pow((1+returnRate/100)*(1-inflationRate/100),targetYear-THIS_YEAR);    // replace 0 with your code
