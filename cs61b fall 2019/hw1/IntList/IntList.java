...
public class IntList {
    ...
    static IntList subTail(IntList L, int start) {
        ...
        while(L!=null){
            if(num==start){
              Final=Head=new IntList(L.head,null);
            }
            if(num>start) {
                Head.tail = new IntList(L.head, null);
                Head = Head.tail;
            }
            L=L.tail;
            num++;
     ...
     *
     * e If the desired items don't exist, or the program
     *      *      *  receives negative START or LEN parameters, the behavior
     *      *      *  of this function is undefined, i.e. you can assum
    ...
    static IntList sublist(IntList L, int start, int len) {
        ...
        if (len>=1) {
            while (temp != null) {
                if (num == start)
                    Final = Head = new IntList(temp.head, null);

                if (num > start && num < start + len) {
                    Head.tail = new IntList(temp.head, null);
                    Head = Head.tail;
                }
                temp = temp.tail;
                num++;
