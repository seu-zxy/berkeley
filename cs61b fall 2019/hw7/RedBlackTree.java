...
public class RedBlackTree<T extends Comparable<T>> {
    ...
    RBTreeNode<T> buildRedBlackTree(BTree.Node<T> r) {
        ...
        if (temp != null) {
            int num = temp.getItemCount();
            for (int i = 0; i < num; i++) {
                T V1 = (T)temp.getItemAt(i);
                insert(V1);
            }
        }
        for (int i = 0; i < temp.getChildrenCount(); i++) {
            BTree.Node temp1 = temp.getChildAt(i);
          buildRedBlackTree(temp1);
        }
        return root;
    }
    ...
    RBTreeNode<T> rotateRight(RBTreeNode<T> node) {
        ...
        boolean old = node.isBlack;
        RBTreeNode lf = node.left;
        RBTreeNode lfr = lf.right;
        lf.right = node;
        node.left = lfr;
        node.isBlack = false;
        lf.isBlack = old;
    ...
    RBTreeNode<T> rotateLeft(RBTreeNode<T> node) {
        ...
        boolean old = node.isBlack;
        RBTreeNode rig = node.right;
        RBTreeNode rigl = rig.left;
        rig.left = node;
        node.right = rigl;
        node.isBlack = false;
        rig.isBlack = old;
